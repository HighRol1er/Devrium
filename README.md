...
1. JWT 인증 문제 → 미들웨어를 통한 인증 흐름 최적화
- `set-tagName page`에서 사용자가 태그 이름(Tag name)을 지정한 후, JWT를 갱신해야 하는 문제가 있었습니다.
이를 해결하기 위해 setTagName POST API를 통해 사용자가 지정한 태그 이름을 서버로 전달할 경우 
미들웨어를 통해서 새로운 JWT를 클라이언트 세션에 반영하도록 구현해 문제를 해결했습니다.

2. Bundle-Analyzer를 활용한 번들 최적화
