...
### 문제 상황 
  - `set-tagName page`에서 사용자가 태그 이름(Tag name)을 지정한 후, 사용자가 수동으로 재 로그인하여 JWT를 갱신해야 하는 문제

### 해결 과정
  
  - 이를 해결하기 위해 setTagName POST API를 통해 사용자가 지정한 태그 이름을 서버로 전달할 경우 
    미들웨어를 통해서 새로운 JWT를 클라이언트 세션에 즉각 반영하도록 구현해 기존 UX 문제를 해결

## 2. Vercel Scheduled Functions을 활용한 자동화 OpenAI API 구축

### 문제 상황 
- 초기 `setInterval`를 활용한 주기적 작업을 실행했지만 이는 서버의 지속적 부하 및 크론 작업 관리가 어려움

### 해결 과정
- Vercel의 외부 Scheduled Functions를 활용해 서버리스 환경에서 안정적이고 자동화된 OpenAI 댓글 작업 처리 기능을 구현
- 서버리스 환경으로 유지보수성을 높힘
...
- 빌드 과정에서 `write` 페이지의 번들 사이즈가 271KB로 다른 페이지에 비해 월등히 높은 문제 발견
...
최종적으로 기존 번들 크기를 약 88% 감소시켜 성능을 크게 향상

## 4. TanStack Query 와 SSR을 사용한 리팩토링 

### 문제 상황 
  - 기존 카테고리 페이지는 5개의 정적 페이지로 각각 구현되어 있었고 이 방식은 다음과 같은 문제가 발생
  
  **A**. 각 페이지의 동일한 로직이 반복되어 코드 중복
